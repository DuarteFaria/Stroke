...
module.exports = async function smokeTest({ win, dialog, api, token, data }) {
  ...
  await waitFor(`document.body.innerText.includes('Análise pronta.')`, 90000);
  ...
  assert.equal(await run(`!!document.querySelector('.stroke-tools') || !!document.querySelector('.stroke-card')`), false);
  assert.equal(await run(`!!document.querySelector('#timing-summary')`), false);
  ...
  await click('Cancelar seleção');
  await settle();
  await capture('analysis-controls.png');
  checks.push('analysis preserves markers without suggestions', 'inline analysis controls and region selection');
  await click('+ Entrada');
  await waitFor(`document.querySelector('.marker-edit')?.innerText.includes('Confirmado')`);
  const manualId = await run(`document.querySelector('.marker-edit').dataset.eventId`);
  const markerTime = await run(`document.querySelector('.marker-edit time').textContent`);
  await click('Ajustar');
  await click('+1 fotograma');
  await waitFor(`document.querySelector('.marker-edit time').textContent !== ${JSON.stringify(markerTime)}`);
  await click('Concluir');
  await run(`document.querySelector('button[aria-label="Anular"]').click()`);
  await waitFor(`document.querySelector('.marker-edit')?.innerText.includes('Por rever')`);
  await click('Confirmar');
  await click('Remover');
  await waitFor(`!document.querySelector('.stroke-marker[data-event-id="${manualId}"]')`);
  await run(`document.querySelector('button[aria-label="Anular"]').click()`);
  await waitFor(`!!document.querySelector('.stroke-marker[data-event-id="${manualId}"]')`);
  await run(`document.querySelector('button[aria-label="Fechar edição da marcação"]').click()`);
  await run(`(() => { const v = document.querySelector('video'); v.currentTime += .3; v.dispatchEvent(new Event('timeupdate')); })()`);
  await settle();
  await click('+ Saída');
  await click('Resumo');
  await waitFor(`[...document.querySelectorAll('.timing-summary dd')].some(x => x.textContent !== '—')`);
  assert.equal(await run(`!!document.querySelector('.workspace-nav button[aria-pressed="true"]')`),true);
  await settle();
  await capture('timing-summary.png');
  await click('Editar');
  await run(`document.querySelector('button[aria-label="Fechar edição da marcação"]').click()`);
  await settle();
  await capture('stroke-timeline.png');
  assert.equal(await run(`document.documentElement.scrollWidth <= document.documentElement.clientWidth`), true);
  win.setSize(1024, 768);
  await settle();
  assert.equal(await run(`document.documentElement.scrollWidth <= document.documentElement.clientWidth`), true);
  await capture('stroke-timeline-compact.png');
  win.setSize(1440, 940);
  await settle();
  await run(`document.querySelector('.stroke-marker').click()`);
  await click('Ajustar');
  await settle();
  await capture('stroke-adjustment.png');
  await run(`document.querySelector('button[aria-label="Fechar edição da marcação"]').click()`);
  checks.push('visible body tools', 'manual marker', 'frame adjustment', 'confirmation and removal undo');
  ...
  await waitFor(`document.querySelector('input[name="stroke-mode"][value="correct"]')?.checked`);
  await settle();
  ...
  assert(project.events.some(e => e.source === 'manual' && e.review === 'confirmed'));
